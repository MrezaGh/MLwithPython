...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = ((activated_output - y) * output_layer_activation_derivative(output)).item()
        hidden_layer_error = output_layer_error * np.multiply(
            self.hidden_to_output_weights.reshape(-1, 1),
            np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input))

        # bias_gradients = np.matrix(output_layer_error).repeat(3, axis=0)
        ...
        self.hidden_to_output_weights -= self.learning_rate * hidden_to_output_weight_gradients.reshape(1, 3)
    ...
    def train_neural_network(self):
        ...
        for epoch in range(self.epochs_to_train):
            for x,y in self.training_points:
                ...
                # print("input_to_hidden_weights", self.input_to_hidden_weights)
                # print("hidden_to_output_weights", self.hidden_to_output_weights)
